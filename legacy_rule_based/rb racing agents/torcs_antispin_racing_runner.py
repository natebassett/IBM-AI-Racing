...
#############################################
# TORCS RULE-BASED ANTI-SPIN RACING RUNNER
#############################################
...
def get_dynamic_target_speed(S, prev_front=None):
    ...
    if phase == "EXIT":
        return min(TARGET_SPEED, base_speed + 12)
...
def calculate_throttle(S, brake, steer, prev_front=None):
    ...
    # Exit: accelerate only when the car is settled.
    if plan["phase"] == "EXIT":
        if abs(S["angle"]) > 0.22 or abs(S["speedY"]) > 5.5:
            return 0.10
        if speed < target - 25:
            return 0.85
        if speed < target - 8:
