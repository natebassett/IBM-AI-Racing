...
#############################################
# TORCS RULE-BASED ANTI-SPIN RACING RUNNER V1.1
#############################################
...
TARGET_SPEED = 225
FAST_BEND_SPEED = 205
SHALLOW_BEND_SPEED = 185
MEDIUM_CORNER_SPEED = 145
TIGHT_CORNER_SPEED = 98
HAIRPIN_SPEED = 68
...
LATERAL_SPEED_LIMIT = 15.5
HIGH_LATERAL_SPEED_LIMIT = 25.0
LATERAL_STEER_DAMPING = 0.011
...
def get_dynamic_target_speed(S, prev_front=None):
    ...
    # - exit: accelerate out
    if phase == "APPROACH":
        return max(HAIRPIN_SPEED, base_speed - 12)

    if phase == "APEX":
        return max(HAIRPIN_SPEED, base_speed - 4)

    if phase == "EXIT":
        return min(TARGET_SPEED, base_speed + 18)
...
def calculate_brake(S, steer, prev_front=None):
    ...
    if plan["phase"] == "APPROACH" and speed > target + 10:
        # Slightly lighter trail braking than the original so the car
        # does not throw away speed on the first large bend.
        return 0.52
