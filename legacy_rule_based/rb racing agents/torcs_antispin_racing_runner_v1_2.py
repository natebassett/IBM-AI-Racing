...
#############################################
# TORCS RULE-BASED ANTI-SPIN RACING RUNNER V1.2
#############################################
...
def get_predictive_front_speed_cap(S, prev_front=None):
    ...
    # Only treat it as dangerous when the visible road is shrinking.
    closing_fast = front_delta < -0.65

    if not closing_fast or speed < 120:
        return TARGET_SPEED
...
def get_high_speed_entry_risk(S, prev_front=None):
    ...
    # This is deliberately not based on anglePlan because the problem bend
    # still looked straight through the centre sensor until very late.
    return speed > 150 and front < 115 and front_delta < -0.65
...
def calculate_brake(S, steer, prev_front=None):
    ...
    # High-speed entry risk: brake earlier, but not as a last-second panic.
...
def calculate_throttle(S, brake, steer, prev_front=None):
    ...
    if get_high_speed_entry_risk(S, prev_front) and speed > target - 5:
        return 0.0
    ...
    if plan["phase"] == "APEX":
        if plan["section"] in ["FAST_BEND", "SHALLOW"] and abs(S["angle"]) < 0.25 and abs(S["speedY"]) < 4.5:
            return 0.32 if speed < target - 10 else 0.14
