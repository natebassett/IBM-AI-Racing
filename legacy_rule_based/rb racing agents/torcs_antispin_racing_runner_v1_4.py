...
#############################################
# TORCS RULE-BASED ANTI-SPIN RACING RUNNER V1.3
#############################################
...
TARGET_SPEED = 214
FAST_BEND_SPEED = 198
SHALLOW_BEND_SPEED = 174
MEDIUM_CORNER_SPEED = 132
TIGHT_CORNER_SPEED = 92
HAIRPIN_SPEED = 66
...
STEER_SMOOTHING = 0.87
MAX_STEER_CHANGE = 0.052
...
def get_desired_track_pos(S, plan):
    ...
    if phase == "APEX":
        ...
        return direction * min(0.08, outside * 0.35)

    if phase == "EXIT":
        # Do not snap straight back to centre; unwind the line smoothly.
        return -direction * min(0.04, outside * 0.18)
...
def calculate_steering(S, previous_steer=0.0, prev_front=None):
    ...
    if is_stability_mode(S):
        ...
        if abs_pos < 0.58:
            ...
            # Shallow bends were twitching because the chosen sensor flips between
            # centre and near-centre rays. Reduce that small nudge at speed.
            if plan["section"] in ["FAST_BEND", "SHALLOW"] and speed > 85:
                sensor_steer *= 0.55
...
def calculate_throttle(S, brake, steer, prev_front=None):
    ...
    if plan["phase"] == "APEX":
        if plan["section"] in ["FAST_BEND", "SHALLOW"] and abs(S["angle"]) < 0.25 and abs(S["speedY"]) < 4.5:
            ...
            if plan["front"] > 70 and speed < target - 8:
                return 0.36
            return 0.30 if speed < target - 12 else 0.16
        if speed < target - 18 and abs(S["angle"]) < 0.30 and abs(S["speedY"]) < 3.5:
            return 0.22
        return 0.10
    ...
    if speed < target - 35:
        if plan["section"] in ["SHALLOW", "MEDIUM"] and speed > 80:
            return 0.82
        return 1.00
...
def smooth_accel(previous_accel, desired_accel, brake, S, plan):
    """
    Smooth throttle changes only; braking still cuts throttle immediately.

    This avoids the 0.48/0.60/0.48 oscillation seen in corner exit logs,
    which made the car feel jerky without giving a real lap-time benefit.
    """
    if brake > 0:
        return 0.0

    if plan["section"] in ["MEDIUM", "TIGHT", "HAIRPIN"] or plan["phase"] in ["APEX", "EXIT"]:
        max_up = 0.075
        max_down = 0.18
    elif abs(S["speedY"]) > 3.5 or abs(S["angle"]) > 0.16:
        max_up = 0.09
        max_down = 0.20
    else:
        max_up = 0.16
