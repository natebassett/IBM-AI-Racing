...
#############################################
# TORCS RULE-BASED ANTI-SPIN RACING RUNNER V1.5 V1.3
#############################################
...
def validate_parameters():
    print("\n========== TORCS RULE-BASED ANTI-SPIN RACING RUNNER V1.6 ==========")
...
def get_emergency_front_brake(S):
    ...
    if front < 8 and speed > 30:
        return 0.65 if controlled_bend and speed < 70 else 1.00
    if front < 14 and speed > 55:
        return 0.42 if controlled_bend and speed < 76 else 1.00
...
def get_dynamic_target_speed(S, prev_front=None):
    ...
    controlled_bend = is_controlled_low_speed_bend(S)
    if front < 10:
        return 58 if controlled_bend else 42
    if front < 18:
        return 76 if controlled_bend else 60
    if front < 28:
        return 92 if controlled_bend else 82
    if front < 42:
        return 112 if controlled_bend else 105
...
def calculate_steering(S, previous_steer=0.0, prev_front=None):
    ...
    if is_stability_mode(S):
        ...
        if abs_pos < 0.58:
            ...
            if plan["section"] in ["FAST_BEND", "SHALLOW"] and speed > 95:
                sensor_steer *= 0.75
