...
#############################################
# TORCS RULE-BASED ANTI-SPIN RACING RUNNER V1.8
#############################################
...
def get_corner_phase(S, section, front, front_delta):
    ...
    # V1.8: let the car recognise exit earlier. V1.7 kept many bends in
    # APEX until the centre sensor exceeded 42m, causing it to coast/brake
    # for too long after the car was already pointed out of the bend.
...
def calculate_throttle(S, brake, steer, prev_front=None):
    ...
    if plan["phase"] == "EXIT":
        if abs(S["angle"]) > 0.30 or abs(S["speedY"]) > 6.4:
            return 0.14
        if speed < target - 28:
            return 0.92
        if speed < target - 10:
