...
#############################################
# TORCS RULE-BASED ANTI-SPIN RACING RUNNER V2
#############################################
...
def create_csv_logger():
    ...
    filename = os.path.join(
        script_dir,
        "torcs_antispin_v2_log_" + datetime.now().strftime("%Y%m%d_%H%M%S") + ".csv"
    )
...
def get_dynamic_target_speed(S, prev_front=None):
    ...
    # - exit: accelerate out
    if phase == "APPROACH":
        return max(HAIRPIN_SPEED, base_speed - 14)

    if phase == "APEX":
        return max(HAIRPIN_SPEED, base_speed - 4)

    if phase == "EXIT":
        return min(TARGET_SPEED, base_speed + 24)
...
def calculate_throttle(S, brake, steer, prev_front=None):
    ...
    # Safer launch.
    if speed < 30:
        if angle_abs > 0.25 or abs(S["trackPos"]) > 0.45:
            return 0.20
        return 0.45
