...
#############################################
# TORCS RULE-BASED ANTI-SPIN RACING RUNNER V3
#############################################
...
TARGET_SPEED = 220
FAST_BEND_SPEED = 202
SHALLOW_BEND_SPEED = 184
MEDIUM_CORNER_SPEED = 142
TIGHT_CORNER_SPEED = 96
HAIRPIN_SPEED = 68
...
CENTERING_GAIN = 0.38
SENSOR_STEER_GAIN = 0.09

# Racing line offset: conservative but allows better use of track.
MAX_OUTSIDE_OFFSET = 0.34
MEDIUM_OUTSIDE_OFFSET = 0.24
TIGHT_OUTSIDE_OFFSET = 0.16

STEER_SMOOTHING = 0.835
MAX_STEER_CHANGE = 0.065
STEER_DEADZONE = 0.010

SAFE_TRACK_LIMIT = 0.76
HARD_TRACK_LIMIT = 0.92

# Edge speed trimming: prevents charge-to-edge then panic braking.
EDGE_WARN_LIMIT = 0.55
EDGE_TRIM_LIMIT = 0.65

# Planner tuning: when front is short but side path is open,
# treat it as a bend to take, not automatically a hairpin.
OPEN_SIDE_RATIO = 1.75
...
def create_csv_logger():
    ...
    filename = os.path.join(
        script_dir,
        "torcs_antispin_v3_log_" + datetime.now().strftime("%Y%m%d_%H%M%S") + ".csv"
    )
...
def get_best_sensor(S):
    ...
    # Normal running: use forward-ish sensors.
    # If the centre-front is closing, include wider sensors so the car can
    # see the actual path through the bend instead of assuming hairpin.
    if front < 28:
        candidate_indices = range(1, 18)
    elif front < 65:
        candidate_indices = range(3, 16)
    else:
        candidate_indices = range(6, 13)

    best_i = 9
    best_score = track[9] * 1.08

    for i in candidate_indices:
        distance = track[i]
        offset = abs(i - 9)

        # Normal centre preference.
        if front < 28:
            # Let side paths win when front is very short.
            centre_penalty = 1.0 - (offset * 0.018)
        elif front < 65:
...
def get_emergency_front_brake(S):
    ...
    # Use current best sensor to avoid false panic on shallow bends.
    best_i = get_best_sensor(S)
    best_distance = S["track"][best_i]
    side_path_open = best_distance > max(front * OPEN_SIDE_RATIO, front + 18)

    if front < 8 and speed > 30:
        return 0.70 if side_path_open else 1.00
    if front < 14 and speed > 55:
        return 0.55 if side_path_open else 1.00
    if front < 22 and speed > 85:
        return 0.45 if side_path_open else 1.00
    if front < 32 and speed > 110:
        return 0.35 if side_path_open else 0.90
    if front < 45 and speed > 138:
        return 0.25 if side_path_open else 0.70
...
def classify_section(front, planned_angle_abs, best_distance):
    ...
    if front > 155 and planned_angle_abs <= 8:
        section = "STRAIGHT"
    elif front > 125 and planned_angle_abs <= 11:
        section = "FAST_BEND"
    elif severity < 0.30:
        section = "FAST_BEND"
    elif severity < 0.50:
    ...
    # If front is short but the best side path is much longer,
    # it is probably a bend with an escape line, not a dead-end hairpin.
    side_path_open = best_distance > max(front * OPEN_SIDE_RATIO, front + 18)
    ...
    if front < 18:
        if side_path_open:
            ...
            section = "HAIRPIN"
    elif front < 34 and planned_angle_abs >= 2.5:
        if side_path_open:
            section = "MEDIUM"
        else:
            section = "TIGHT"
    elif front < 55 and section in ["STRAIGHT", "FAST_BEND", "SHALLOW"]:
        section = "MEDIUM"
...
def get_corner_phase(S, section, front, front_delta):
    ...
    lateral_abs = abs(S["speedY"])

    if section in ["STRAIGHT", "FAST_BEND"]:
        return "STRAIGHT"

    # Exit earlier when the car is stable and the road is no longer closing hard.
    # This fixes the log issue where the car stayed in APEX too long.
    if front > 42 and front_delta > -0.35 and angle_abs < 0.20 and lateral_abs < 5.5:
        return "EXIT"

    if front > 55 and angle_abs < 0.16 and lateral_abs < 4.0:
        return "EXIT"

    # Approach when the road ahead is closing and speed is still above the corner target.
    if front_delta < -1.0 and speed > get_section_speed(section) + 8:
        return "APPROACH"

    if speed > get_section_speed(section) + 15 and front < 70:
        return "APPROACH"

    # True apex: tight front, meaningful rotation, or not yet settled.
    if front < 38 or angle_abs > 0.36 or lateral_abs > 7.0:
        return "APEX"

    # If none of the danger signs are present, start exiting.
    return "EXIT"
...
def is_stability_mode(S):
    ...
    V2 keeps the previous safety idea, but avoids treating normal fast corner
    exit as instability too early.
    """
    ...
    if lateral_speed > LATERAL_SPEED_LIMIT and angle_abs > 0.28:
        return True

    if lateral_speed > forward_speed * 0.38 and forward_speed > 55:
        return True
...
def get_dynamic_target_speed(S, prev_front=None):
    ...
    phase = plan["phase"]
    best_distance = plan["best_distance"]
    ...
    # Progressive edge speed trimming.
    # This stops the accelerate -> nearly off-track -> panic pause cycle.
    edge_cap = TARGET_SPEED
    if abs_pos > HARD_TRACK_LIMIT:
        edge_cap = 50
    elif abs_pos > SAFE_TRACK_LIMIT:
        edge_cap = 82
    elif abs_pos > EDGE_TRIM_LIMIT:
        edge_cap = 118
    elif abs_pos > EDGE_WARN_LIMIT:
        edge_cap = 155

    side_path_open = best_distance > max(front * OPEN_SIDE_RATIO, front + 18)

    # Direct speed cap from front distance.
    # If a side path is open, do not drop to 42/60 instantly.
    if front < 10:
        front_cap = 65 if side_path_open else 42
    elif front < 18:
        front_cap = 82 if side_path_open else 60
    elif front < 28:
        front_cap = 105 if side_path_open else 78
    elif front < 42:
        front_cap = 120 if side_path_open else 95
    else:
        front_cap = TARGET_SPEED
    ...
    # - exit: accelerate out
    if phase == "APPROACH":
        phase_speed = max(HAIRPIN_SPEED, base_speed - 12)
    elif phase == "APEX":
        phase_speed = max(HAIRPIN_SPEED, base_speed - 2)
    elif phase == "EXIT":
        phase_speed = min(TARGET_SPEED, base_speed + 22)
    else:
        phase_speed = base_speed

    return min(edge_cap, front_cap, phase_speed)
...
def get_desired_track_pos(S, plan):
    ...
    if direction == 0 or section in ["STRAIGHT", "FAST_BEND"]:
        return 0.0
    ...
    # If the corner is to the right, outside is left (-).
    ...
    if phase == "APEX":
        # Small movement towards inside/apex, but conservative to avoid invalid laps.
        return direction * min(0.12, outside * 0.5)
...
def calculate_steering(S, previous_steer=0.0, prev_front=None):
    ...
    if is_stability_mode(S):
        ...
        angle_correction = S["angle"] * ANGLE_STEER_GAIN / math.pi
        ...
        if abs_pos > HARD_TRACK_LIMIT:
            ...
            pos_gain = CENTERING_GAIN * 0.55
        elif speed > 120:
            pos_gain = CENTERING_GAIN * 0.75
        else:
            pos_gain = CENTERING_GAIN
...
def calculate_brake(S, steer, prev_front=None):
    ...
    if abs_pos > HARD_TRACK_LIMIT and speed > 30:
        return 1.00

    if abs_pos > SAFE_TRACK_LIMIT and speed > 80:
        return 0.65

    if abs_pos > EDGE_TRIM_LIMIT and speed > 135:
        return 0.35

    if abs_pos > EDGE_WARN_LIMIT and speed > 170:
        return 0.22

    # Trail braking into corner approach.
    if plan["phase"] == "APPROACH" and speed > target + 6:
        return 0.62
...
def calculate_throttle(S, brake, steer, prev_front=None):
    ...
    # Do not accelerate while clearly unstable.
    if is_stability_mode(S):
        return 0.0

    # Keep anti-spin, but allow mild lateral movement on fast exits.
    if lateral_abs > 12.0:
        return 0.0
    if lateral_abs > 8.0:
        return 0.10

    if plan["front"] < 12 and plan["best_distance"] < 25:
        return 0.0

    # Progressive edge throttle limit.
    if abs_pos > HARD_TRACK_LIMIT:
        edge_throttle_cap = 0.00
    elif abs_pos > SAFE_TRACK_LIMIT:
        edge_throttle_cap = 0.08
    elif abs_pos > EDGE_TRIM_LIMIT:
        edge_throttle_cap = 0.22
    elif abs_pos > EDGE_WARN_LIMIT:
        edge_throttle_cap = 0.45
    else:
        edge_throttle_cap = 1.00

    # Safer launch.
    if speed < 30:
        if angle_abs > 0.25 or abs_pos > 0.45:
            return min(0.20, edge_throttle_cap)
        return min(0.45, edge_throttle_cap)

    if speed < 80:
        if angle_abs > 0.30:
            return min(0.25, edge_throttle_cap)
        return min(0.65, edge_throttle_cap)

    # Apex: do not just coast forever. If shallow/medium and stable, feed throttle.
    if plan["phase"] == "APEX":
        if plan["section"] in ["SHALLOW", "MEDIUM"] and angle_abs < 0.24 and lateral_abs < 5.5:
            if speed < target - 20:
                return min(0.55, edge_throttle_cap)
            if speed < target - 8:
                return min(0.28, edge_throttle_cap)
            return min(0.12, edge_throttle_cap)

        if speed < target - 18 and angle_abs < 0.32 and lateral_abs < 6.0:
            return min(0.32, edge_throttle_cap)

        return min(0.06, edge_throttle_cap)

    # Exit: more confident throttle, but only when settled.
    if plan["phase"] == "EXIT":
        if angle_abs > 0.28 or lateral_abs > 6.5:
            return min(0.14, edge_throttle_cap)
        if speed < target - 28:
            return min(1.00, edge_throttle_cap)
        if speed < target - 12:
            return min(0.72, edge_throttle_cap)
        if speed < target - 4:
            return min(0.36, edge_throttle_cap)
        return min(0.18, edge_throttle_cap)

    # Straight / fast bend.
    if speed < target - 35:
        return min(1.00, edge_throttle_cap)
    if speed < target - 15:
        return min(0.75, edge_throttle_cap)
    if speed < target - 5:
        return min(0.35, edge_throttle_cap)
...
def should_shutdown(S, stuck_counter, frozen_counter=0):
    ...
    if frozen_counter > 250:
        return True, "telemetry frozen"
...
def drive_rule_based(c, step, csv_writer=None):
    ...
    # Detect frozen telemetry/server state so logs do not fill with repeated rows.
    current_signature = (
        round(S.get("speedX", 0), 3),
        round(S.get("trackPos", 0), 3),
        round(S.get("angle", 0), 3),
        round(S.get("track", [0]*19)[9], 3),
    )

    if getattr(c, "last_signature", None) == current_signature:
        c.frozen_counter += 1
    else:
        c.frozen_counter = 0
        c.last_signature = current_signature

    prev_front = getattr(c, "prev_front", None)

    shutdown, reason = should_shutdown(S, c.stuck_counter, c.frozen_counter)
    ...
    accel = calculate_throttle(S, brake, steer, prev_front)
