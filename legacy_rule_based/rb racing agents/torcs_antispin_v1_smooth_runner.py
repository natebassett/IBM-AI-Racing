...
#############################################
# TORCS RULE-BASED ANTI-SPIN RACING RUNNER V1 SMOOTH
#############################################
...
def create_csv_logger():
    ...
    filename = os.path.join(
        script_dir,
        "torcs_antispin_v1_smooth_log_" + datetime.now().strftime("%Y%m%d_%H%M%S") + ".csv"
    )
...
def classify_section(front, planned_angle_abs, best_distance):
    ...
    if front < 18:
        section = "HAIRPIN"
    elif front < 30 and planned_angle_abs >= 3.5:
        section = "TIGHT"
...
def get_corner_phase(S, section, front, front_delta):
    ...
    # Keep V1 behaviour, but exit shallow/medium bends a little earlier
    # once the car is settled. This avoids unnecessary coasting.
    if section in ["SHALLOW", "MEDIUM"]:
        if front > 48 and front_delta > -0.45 and angle_abs < 0.24 and lateral_abs < 6.0:
            return "EXIT"
    ...
    if front < 42 or angle_abs > 0.42 or lateral_abs > 7.5:
        return "APEX"

    if front_delta > 0.35 and angle_abs < 0.32 and lateral_abs < 6.5:
        return "EXIT"
...
def get_dynamic_target_speed(S, prev_front=None):
    ...
    # - exit: accelerate out
    if phase == "APPROACH":
        return max(HAIRPIN_SPEED, base_speed - 16)

    if phase == "APEX":
        return max(HAIRPIN_SPEED, base_speed - 6)

    if phase == "EXIT":
        return min(TARGET_SPEED, base_speed + 16)
...
def calculate_brake(S, steer, prev_front=None):
    ...
    if is_stability_mode(S):
        # Controlled anti-spin braking. Enough to settle the car,
        # not enough to lock it into a repeated stop.
...
def calculate_throttle(S, brake, steer, prev_front=None):
    ...
    # Apex: V1-style coast, but allow light throttle in shallow/medium bends
    # when stable so it does not lose too much speed.
    if plan["phase"] == "APEX":
        if plan["section"] in ["SHALLOW", "MEDIUM"] and angle_abs < 0.26 and lateral_abs < 5.8:
            if speed < target - 18:
                return 0.38
            if speed < target - 8:
                return 0.20
        ...
        if speed < target - 15 and angle_abs < 0.35:
            return 0.24
        return 0.06

    # Exit: smoother than V1 but not as aggressive as V2/V3.
    if plan["phase"] == "EXIT":
        if angle_abs > 0.26 or lateral_abs > 6.2:
            return 0.10
        if speed < target - 25:
            return 0.82
        if speed < target - 10:
