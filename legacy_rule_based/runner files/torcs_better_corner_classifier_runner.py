...
def get_track_features(S):
    """
    Extracts more reliable corner features from TORCS sensors.

    Instead of using imbalance alone, this uses:
    - front distance
    - best available path distance
    - best path direction
    - steering demand towards the best path
    """
    track = S["track"]

    front = track[9]

    # Focus on usable forward sensors, not extreme side sensors.
    useful_indices = list(range(4, 15))
    best_i = max(useful_indices, key=lambda i: track[i])
    best_distance = track[best_i]

    # Sensor index 9 is straight ahead.
    best_offset = abs(best_i - 9)

    # Weighted corner strength:
    # - higher if the best path is far away from centre
    # - higher if the front distance is short
    # - lower if there is still a long open path ahead
    steer_need = best_offset / 6.0
    closing_factor = max(0.0, (120.0 - front) / 120.0)
    open_path_factor = max(0.0, (140.0 - best_distance) / 140.0)

    corner_strength = (
        0.50 * steer_need
        + 0.35 * closing_factor
        + 0.15 * open_path_factor
    )

    return front, best_distance, best_i, steer_need, corner_strength


def classify_corner(S):
    """
    More stable corner classifier.

    Important:
    - Hairpin requires short front distance AND high steering demand.
    - Light bends are allowed to be fast if the best path is still open.
    """
    front, best_distance, best_i, steer_need, corner_strength = get_track_features(S)

    # Really tight / hairpin:
    # road directly ahead is closing AND the open path is sharply off-centre.
    if front < 32 and steer_need > 0.65:
        return "HAIRPIN"

    if front < 42 and steer_need > 0.50:
        return "HAIRPIN"

    # Tight corner:
    if front < 55 and steer_need > 0.35:
        return "TIGHT"

    if corner_strength > 0.62 and front < 75:
        return "TIGHT"

    # Medium corner:
    if front < 85 and steer_need > 0.22:
        return "MEDIUM"

    if corner_strength > 0.45:
        return "MEDIUM"

    # Shallow bend:
    if front < 130 and steer_need > 0.10:
        return "SHALLOW"

    if corner_strength > 0.25:
        return "SHALLOW"

    # Fast bend:
    if front < 180 or steer_need > 0.04:
        return "FAST_BEND"
...
def print_drive_debug(S, R, step):
    ...
    front, best_distance, best_i, steer_need, strength = get_track_features(S)
    ...
    print(
        ...
        f"front={front:6.1f} | "
        f"best={best_i:02d}:{best_distance:5.1f} | "
        f"need={steer_need:4.2f} | "
