...
TARGET_SPEED = 155          # Safer starting target speed in km/h.
STEER_GAIN = 24             # Steering sensitivity. 20-30 is a safer range.
CENTERING_GAIN = 0.65       # How strongly the car returns to track centre.
BRAKE_THRESHOLD = 0.12      # Higher = brakes less often. Lower = brakes earlier.
ENABLE_TRACTION_CONTROL = True
ENABLE_DEBUG_OUTPUT = True

GEAR_SPEEDS = [0, 25, 55, 90, 130, 175]
...
def calculate_steering(S):
    """
    Steering combines:
    - angle: where the car is pointing relative to the track
    - trackPos: how far left/right the car is from centre
    """
    angle_correction = S["angle"] * STEER_GAIN / math.pi
    centre_correction = S["trackPos"] * CENTERING_GAIN

    steer = angle_correction - centre_correction
...
def get_dynamic_target_speed(S):
    """
    Returns a speed target based on the road ahead.

    TORCS track sensors:
    - S["track"][9] points roughly straight ahead.
    - Sensors around [7], [8], [10], [11] help detect bends before the car reaches them.

    The aim:
    - Go fast on straights.
    - Brake early for tight corners.
    - Keep speed through light bends.
    """
    track = S["track"]

    front = track[9]
    front_left = track[7] + track[8]
    front_right = track[10] + track[11]

    corner_imbalance = abs(front_left - front_right)
    corner_strength = corner_imbalance / max(front, 1.0)

    # Very sharp corner or wall quickly approaching.
    if corner_strength > 1.20 or front < 45:
        return 70

    # Medium corner ahead.
    if corner_strength > 0.70 or front < 70:
        return 95

    # Light bend ahead.
    if corner_strength > 0.35 or front < 100:
        return 125

    # Straight / open section.
    return TARGET_SPEED
...
def calculate_brake(S, steer):
    ...
    # Emergency correction if the car is badly angled at speed.
    if speed > 90 and corner_severity > 0.75:
        return 1.00

    # Brake based on how far above the safe target speed the car currently is.
    if speed > dynamic_target + 35:
        return 0.95

    if speed > dynamic_target + 20:
        return 0.70
...
def calculate_throttle(S, brake, steer):
    ...
    # Never accelerate while braking.
    if brake > 0.0:
        return 0.0

    # If the car is heavily angled, don't keep pushing.
    if corner_severity > 0.55:
        return 0.10

    # Full acceleration when safely below dynamic target.
    if speed < dynamic_target - 25:
        return 1.00

    # Controlled acceleration near the target.
    if speed < dynamic_target - 8:
        return 0.55
