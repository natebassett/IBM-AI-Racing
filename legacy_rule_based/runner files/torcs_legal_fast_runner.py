...
def calculate_steering(S, previous_steer=0.0):
    ...
    if S["trackPos"] > SAFE_TRACK_LIMIT:
        raw_steer -= 0.35
...
def calculate_brake(S, steer):
    ...
    # If close to edge, brake to stop track cutting / off-track moments.
    if abs(S["trackPos"]) > SAFE_TRACK_LIMIT and speed > 70:
        return 0.65
...
def calculate_throttle(S, brake, steer):
    ...
    # Do not accelerate hard when near track limits.
