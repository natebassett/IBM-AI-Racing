...
#############################################
# TORCS LOOKAHEAD PLANNER RUNNER
#############################################
...
def plan_next_curve(S):
    ...
    # Forward-focused sensors only.
    # This stops side sensors making a straight look like a corner.
    candidate_indices = list(range(5, 14))
    ...
    for i in candidate_indices:
        ...
        # Prefer centre unless a side path is clearly better.
        centre_preference = 1.0 - (offset * 0.07)
        score = distance * centre_preference
    ...
    closing = max(0.0, (140.0 - front) / 140.0)
    angle_factor = planned_angle_abs / 19.0
    distance_factor = max(0.0, (130.0 - best_distance) / 130.0)
    ...
    # Straights should stay straights unless the front road is genuinely closing.
    if front > 150 and planned_angle_abs <= 4:
        section = "STRAIGHT"
    elif front > 125 and planned_angle_abs <= 7:
        section = "FAST_BEND"
    elif severity < 0.24:
        section = "FAST_BEND"
    elif severity < 0.42:
    ...
    # Hairpin must have both short front and a clearly off-centre planned path.
    if section == "HAIRPIN" and not (front < 55 and planned_angle_abs >= 7):
        section = "TIGHT"

    # If the front is open, never classify as tight/hairpin.
