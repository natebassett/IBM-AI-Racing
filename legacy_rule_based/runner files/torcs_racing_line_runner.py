...
TARGET_SPEED = 215          # Safer starting target speed in km/h.
STEER_GAIN = 18             # Steering sensitivity. 20-30 is a safer range.
CENTERING_GAIN = 0.30       # How strongly the car returns to track centre.
...
def get_racing_line_target_pos(S):
    """
    Creates a pseudo racing line.

    Instead of forcing trackPos to 0, it positions the car towards the outside
    before/through a corner. This allows wider entry and smoother cornering.
    ...
    # Right corner: prepare from left side.
    if corner_direction == 1:
        return -MAX_RACING_LINE_OFFSET

    # Left corner: prepare from right side.
    if corner_direction == -1:
        return MAX_RACING_LINE_OFFSET

    # Straight: gradually return near centre, but not aggressively.
    return 0.0
...
def calculate_steering(S, previous_steer=0.0):
    ...
    racing_line_correction = (S["trackPos"] - target_track_pos) * CENTERING_GAIN

    raw_steer = angle_correction - racing_line_correction
    raw_steer = clamp(raw_steer, -1.0, 1.0)

    # Low-pass filter to stop the car snapping left/right.
    smoothed_steer = (
        STEER_SMOOTHING * previous_steer
        + (1.0 - STEER_SMOOTHING) * raw_steer
    )

    return clamp(smoothed_steer, -1.0, 1.0)
...
def get_dynamic_target_speed(S):
    """
    Aggressive dynamic speed target.

    This allows 200+ km/h on straights, but still reduces speed when the
    track sensors show a corner/wall coming up.
    """
    track = S["track"]

    front = track[9]
    left_ahead = track[6] + track[7] + track[8]
    right_ahead = track[10] + track[11] + track[12]

    corner_imbalance = abs(left_ahead - right_ahead)
    corner_strength = corner_imbalance / max(front, 1.0)

    # Hairpin / very tight corner.
    if corner_strength > 1.45 or front < 38:
        return 75

    # Sharp corner.
    if corner_strength > 0.95 or front < 60:
        return 105

    # Medium corner.
    if corner_strength > 0.55 or front < 90:
        return 140

    # Fast bend.
    if corner_strength > 0.28 or front < 130:
        return 170
...
def calculate_brake(S, steer):
    ...
    if speed > 120 and corner_severity > 0.82:
        return 1.00

    # Strong braking if arriving much too fast.
    if speed > dynamic_target + 45:
        return 0.95

    if speed > dynamic_target + 28:
        return 0.70

    if speed > dynamic_target + 14:
        return 0.38
...
def calculate_throttle(S, brake, steer):
    ...
    # Keep a little throttle through mild bends for stability.
    if corner_severity > 0.65:
        return 0.08
