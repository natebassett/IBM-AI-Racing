...
#############################################
# TORCS RULE-BASED RACING PHASE RUNNER
#############################################
...
def create_csv_logger():
    ...
    filename = os.path.join(
        script_dir,
        "torcs_racing_phase_log_" + datetime.now().strftime("%Y%m%d_%H%M%S") + ".csv"
    )
...
def is_stability_mode(S):
    # From your log: loss of control showed angle climbing past 1.0+ while speed fell.
    return abs(S["angle"]) > 0.85 or abs(S["speedY"]) > max(18.0, abs(S["speedX"]) * 0.45)
...
def get_dynamic_target_speed(S, prev_front=None):
    ...
    # - exit: accelerate out
    if phase == "APPROACH":
        return max(HAIRPIN_SPEED, base_speed - 18)

    if phase == "APEX":
        return max(HAIRPIN_SPEED, base_speed - 8)

    if phase == "EXIT":
        return min(TARGET_SPEED, base_speed + 22)
...
def calculate_steering(S, previous_steer=0.0, prev_front=None):
    ...
    if is_stability_mode(S):
        ...
        raw_steer = (S["angle"] * 9.0 / math.pi) - (S["trackPos"] * 1.0)
...
def calculate_throttle(S, brake, steer, prev_front=None):
    ...
    # Apex: coast rather than stabbing throttle.
    if plan["phase"] == "APEX":
        if speed < target - 15 and abs(S["angle"]) < 0.35:
            return 0.28
        return 0.08

    # Exit: allow acceleration earlier if car is rotating back.
    if plan["phase"] == "EXIT":
        if speed < target - 20:
            return 1.00
        if speed < target - 6:
