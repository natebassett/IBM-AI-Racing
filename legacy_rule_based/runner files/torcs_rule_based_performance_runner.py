...
#############################################
# TORCS RULE-BASED PERFORMANCE RUNNER
#############################################
...
TARGET_SPEED = 215          # Full straight-line speed.
FAST_BEND_SPEED = 195       # Very light bend.
...
def create_csv_logger():
    ...
    filename = os.path.join(
        script_dir,
        "torcs_rule_based_log_" + datetime.now().strftime("%Y%m%d_%H%M%S") + ".csv"
    )
    ...
    writer.writerow([
        ...
        "centre_bias", "emergency_front_brake", "stuck_counter",
...
def write_csv_log(writer, S, R, step, stuck_counter):
...
def get_emergency_front_brake(S):
    """
    Hard emergency braking based on the front sensor.

    The previous logs showed the car at 140+ km/h while front distance
    collapsed from ~16 down to 0. That must override the normal planner.
    """
    speed = S["speedX"]
    front = S["track"][9]

    if front < 8 and speed > 45:
        return 1.00

    if front < 14 and speed > 70:
        return 1.00

    if front < 22 and speed > 95:
        return 1.00

    if front < 32 and speed > 120:
        return 0.85

    if front < 45 and speed > 145:
        return 0.65
...
def plan_next_curve(S):
    """
    Performance rule-based lookahead planner.

    Key idea:
    - On open straights, heavily prefer centre-forward.
    - When the front sensor collapses, reduce centre bias and let side
      sensors take over earlier.
    """
    track = S["track"]
    front = track[9]

    # The closer the wall is ahead, the less we bias toward centre.
    if front > 145:
        centre_bias = 1.25
    elif front > 100:
        centre_bias = 1.10
    ...
    candidate_indices = list(range(4, 15))

    best_i = 9
    best_score = track[9] * centre_bias

    for i in candidate_indices:
        distance = track[i]
        offset = abs(i - 9)

        # Prefer centre on straights, but allow side sensors if the front is closing.
        centre_penalty = 1.0 - (offset * 0.075)

        # If the front is closing, side sensors are useful escape/lookahead paths.
        if front < 70:
    ...
    severity = (
        0.46 * angle_factor
        + 0.40 * closing
        + 0.14 * distance_factor
    )

    # Straight recognition is deliberately relaxed.
    if front > 150 and planned_angle_abs <= 7:
        section = "STRAIGHT"
    ...
    # Hard front-distance overrides.
    # If front is tiny, do not let centre sensor pretend this is only medium.
    if front < 22 and planned_angle_abs >= 2.5:
        section = "HAIRPIN"
    elif front < 38 and planned_angle_abs >= 4:
        section = "TIGHT"
    elif front < 55 and section in ["FAST_BEND", "SHALLOW"]:
        section = "MEDIUM"

    # But do not call a very open road tight.
    if front > 135 and section in ["TIGHT", "HAIRPIN"]:
        section = "MEDIUM"
    if front > 160 and section == "MEDIUM":
        section = "SHALLOW"
...
def get_dynamic_target_speed(S):
    ...
    # Absolute safety near track limits.
    ...
    # Extra front-distance speed cap.
    # This directly fixes the logged failure where front collapsed at 140+.
    if front < 10:
        return 45
    if front < 18:
        return 60
    ...
    if front < 42:
        return 95
    if front < 60:
        return min(TIGHT_CORNER_SPEED, 105)
...
def calculate_steering(S, previous_steer=0.0):
    ...
    # Low-speed launch/recovery uses the original stable centre logic.
    if speed < 70 or abs(S["trackPos"]) > SAFE_TRACK_LIMIT:
        ...
        sensor_steer = clamp(plan["best_angle"] / 19.0, -1.0, 1.0)

        # If the wall is very close ahead, trust the planned escape direction more.
        if front < 45:
            sensor_weight = 0.95
        elif front < 75:
            sensor_weight = 0.78
        else:
            sensor_weight = STEER_GAIN

        raw_steer = (
            sensor_steer * sensor_weight
            - S["angle"] * ANGLE_GAIN
            - S["trackPos"] * TRACK_POS_GAIN
        )

        # Stronger edge recovery.
        if S["trackPos"] > HARD_TRACK_LIMIT:
            raw_steer -= 1.00
        elif S["trackPos"] < -HARD_TRACK_LIMIT:
            raw_steer += 1.00
    ...
    # Allow slightly faster steering when a wall is close.
    if front < 35:
        max_delta = 0.085
    elif front < 60:
        max_delta = 0.070
...
def calculate_brake(S, steer):
    ...
    # Brake based on overspeed relative to target.
    if speed > target + 42:
        return 1.00
...
def drive_rule_based(c, step, csv_writer=None):
    ...
    if shutdown:
        ...
        R["brake"] = 1.0
        R["meta"] = 1
        write_csv_log(csv_writer, S, R, step, c.stuck_counter)
        return True
