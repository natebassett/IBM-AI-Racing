...
def calculate_steering(S, previous_steer=0.0):
    ...
    # At very low speed, don't allow huge sensor steering.
    # This prevents start-line wall slams.
    sensor_steer = clamp(plan["best_angle"] / 19.0, -0.65, 0.65)

    if speed < 35:
        sensor_steer = clamp(sensor_steer, -0.25, 0.25)
    elif speed < 70:
        sensor_steer = clamp(sensor_steer, -0.45, 0.45)
...
def calculate_throttle(S, brake, steer):
    ...
    # Gentle launch so it doesn't snap into the wall.
    if speed < 30:
        return 0.45
