...
def estimate_corner_severity(S, steer):
    """
    Uses both car angle and final steering demand.
    This helps detect corners before the car fully loses the line.
    """
...
def calculate_brake(S, steer):
    ...
    # Strong braking for serious cornering at high speed.
    if speed > 120 and corner_severity > 0.55:
        return 0.85

    # Medium braking when the car is entering a bend too quickly.
    if speed > 95 and corner_severity > 0.38:
        return 0.55

    # Light braking for smaller bends.
    if speed > 75 and corner_severity > BRAKE_THRESHOLD:
        return 0.30
...
def calculate_throttle(S, brake, steer):
    ...
    # Gentle acceleration while turning.
    if corner_severity > 0.35:
        return 0.25

    # Normal acceleration below target speed.
    if speed < TARGET_SPEED:
        return 0.75

    # Maintain speed rather than constantly flooring it.
    return 0.15
