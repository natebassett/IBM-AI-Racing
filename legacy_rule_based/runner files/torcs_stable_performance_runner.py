...
#############################################
# TORCS RULE-BASED STABLE PERFORMANCE RUNNER
#############################################
...
# Uses the original TORCS steering sign consistently:
# steer = angle correction - track position correction
ANGLE_STEER_GAIN = 18.0
CENTERING_GAIN = 0.42

# Sensor steering is deliberately small. It helps plan, but does not dominate.
SENSOR_STEER_GAIN = 0.18

STEER_SMOOTHING = 0.82
...
def validate_parameters():
    ...
    print("Steering:            original TORCS sign + smoothed correction")
...
def create_csv_logger():
    ...
    filename = os.path.join(
        script_dir,
        "torcs_stable_rule_based_log_" + datetime.now().strftime("%Y%m%d_%H%M%S") + ".csv"
    )
    ...
    writer.writerow([
        ...
        "emergency_front_brake", "stuck_counter",
...
# ================= LOOKAHEAD / SPEED PLANNER =================

def get_best_sensor(S):
    track = S["track"]
    front = track[9]

    # Use forward-ish sensors only.
    # Wider sensors become useful only when front distance is collapsing.
    if front < 65:
    ...
    best_i = 9
    best_score = track[9] * 1.15

    for i in candidate_indices:
        distance = track[i]
        offset = abs(i - 9)

        # Centre is preferred unless a side path is clearly better.
        if front < 65:
            centre_penalty = 1.0 - (offset * 0.04)
        else:
            centre_penalty = 1.0 - (offset * 0.11)
...
def get_emergency_front_brake(S):
    ...
    # Hard front-distance braking from the previous log analysis.
    if front < 8 and speed > 35:
        return 1.00

    if front < 14 and speed > 60:
        return 1.00

    if front < 22 and speed > 90:
        return 1.00

    if front < 32 and speed > 115:
        return 0.90

    if front < 45 and speed > 140:
        return 0.70
...
def plan_next_curve(S):
    ...
    # Never make open track too conservative.
...
def get_dynamic_target_speed(S):
    ...
    # Direct cap from front distance.
    if front < 10:
        return 42
    if front < 18:
        return 60
...
def calculate_steering(S, previous_steer=0.0):
    ...
    # Original TORCS style steering sign.
    angle_correction = S["angle"] * ANGLE_STEER_GAIN / math.pi

    # Stronger centering near edges, softer on straights.
    ...
    if abs_pos > HARD_TRACK_LIMIT:
        ...
        centre_gain = CENTERING_GAIN * 0.55
    elif speed > 120:
        centre_gain = CENTERING_GAIN * 0.75
    else:
        centre_gain = CENTERING_GAIN

    position_correction = S["trackPos"] * centre_gain

    # Sensor steer helps only when not already near the edge.
    # This stops the planner from dragging the car outward repeatedly.
    sensor_steer = 0.0
    if abs_pos < 0.55:
        sensor_steer = clamp(plan["best_angle"] / 19.0, -1.0, 1.0) * SENSOR_STEER_GAIN

    raw_steer = angle_correction - position_correction + sensor_steer

    # Extra legal recovery.
    ...
    # If close to the edge, allow a bit more correction speed.
    if abs_pos > SAFE_TRACK_LIMIT:
        max_delta = 0.080
    elif plan["front"] < 35:
        max_delta = 0.075
    else:
...
def should_shutdown(S, stuck_counter):
    ...
    if abs(S.get("trackPos", 0)) > 1.12:
        return True, "out of bounds"

    if abs(S.get("angle", 0)) > 1.75:
